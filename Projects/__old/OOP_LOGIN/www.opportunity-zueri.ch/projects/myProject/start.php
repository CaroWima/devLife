...
<html>
    ...
    <body>
        ...
        $query      = $connection->query("SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA = 'ipiluwig'");
        $tables     = $query->fetchAll(PDO::FETCH_COLUMN);

        if (empty($tables)) {
            echo '<p class="center">There are no tables in database <code>'.DB_NAME.'</code>.</p>';
        } else {
            echo '<p class="center">Database <code>'.DB_NAME.'</code> contains the following tables:</p>';
